...
ops=torchvision.transforms.ToPILImage()
import cv2
...
# constants
def get_schedule_jump(t_T, n_sample, jump_length, jump_n_sample, #{'jump_length': 10, 'jump_n_sample': 10, 'n_sample': 1, 't_T': 250}
                      jump2_length=1, jump2_n_sample=1,
                      jump3_length=1, jump3_n_sample=1,
                      start_resampling=100000000):
    #Used for multi-step sampling. Used in Function "p_sample()"
    jumps = {}
...
class Conditioning(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # ff-parser in the paper MedSegDiff, for modulating out the high frequencies
        #64 128 128
        dtype = x.dtype
        x = fft2(x)
...
class Unet(nn.Module):
    def __init__(
        ...
        self.corss_att_layer=CrossAttention(4,bn_layer=False)
        self.input_img_channels = input_img_channels+mask_channels
        self.mask_channels = mask_channels
        self.self_condition = self_condition

        output_channels = mask_channels+input_img_channels
        mask_channels = mask_channels * (2 if self_condition else 1)

        init_dim = default(init_dim, dim)
        self.init_conv = nn.Conv2d(self.input_img_channels, init_dim, 7, padding = 3)
    ...
    def forward(
        ...
        h = []
        #x=xt,cond=img---->x=conv(xt),c=conv(cond)
        for (block1, block2, attn, downsample), (cond_block1, cond_block2, cond_attn, cond_downsample), conditioner in zip(self.downs, self.cond_downs, self.conditioners):
            ...
            # Please check Repo MedSegDiff for details.
        ...
        x = self.mid_block1(x, t) #512 16 16
        c = self.cond_mid_block1(c, t) #512 16 16
        ...
        #c = torch.cat((c, c_r),dim = 1)


        x = self.final_res_block(x, t)
        #c = self.final_res_block_c(c, t)

        #final_cat = torch.cat((c,x),dim=1)
        x = self.final_conv(x)
        return x
...
class MedSegDiff(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        return (
            extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t -  #'sqrt_recip_alphas_cumprod', torch.sqrt(1. / alphas_cumprod)
            extract(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape) * noise # 'sqrt_recipm1_alphas_cumprod', torch.sqrt(1. / alphas_cumprod - 1)
        )
    ...
    def q_posterior(self, x_start, x_t, t):
        posterior_mean = (
            extract(self.posterior_mean_coef1, t, x_t.shape) * x_start +  #betas * torch.sqrt(alphas_cumprod_prev) / (1. - alphas_cumprod))
            extract(self.posterior_mean_coef2, t, x_t.shape) * x_t       #(1. - alphas_cumprod_prev) * torch.sqrt(alphas) / (1. - alphas_cumprod))
        )
    ...
    def q_sample(self, x_start, t, noise=None):
        pass
    @torch.no_grad()
    def p_sample(self, x, t, c,cond_mask, full_num=0,x_self_cond = None, clip_denoised = True):
        b, *_, device = *x.shape, x.device  # x=噪音，t从999-->0，c是原图
        batched_times = torch.full((x.shape[0],), t, device=x.device, dtype=torch.long)
        random_flag=random()

###################################Reverse Weighted Inpainting###############################################
        if True: #weighted repaint, if you do not use, True->False
            #weight=1
            weight=1-self.betas[t]   #weight=0.25-->0.99
            parm_type = 2
            pred_mask_channel=x[:,0,:,:].unsqueeze(1)
            pred_img_channel=x[:,1:,:,:]
            c_t=self.q_sample(normalize_to_neg_one_to_one(c),batched_times)
            if parm_type==2:
                c_t_known = c_t*(1-cond_mask)*(1-weight)   +  c_t*cond_mask*(1-weight)
                c_t_unknown= pred_img_channel*cond_mask*weight + pred_img_channel*(1-cond_mask)*weight
            else:
                c_t_known = c_t*(1-cond_mask)*weight#   +  c_t*cond_mask*(1-weight)
                c_t_unknown= pred_img_channel*cond_mask*weight# + pred_img_channel*(1-cond_mask)*(1-weight)

            ct_fused=c_t_unknown+c_t_known

            x = torch.cat((pred_mask_channel,ct_fused),dim=1)

###################################Reverse Weighted Inpainting###############################################



        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, c = c, x_self_cond = x_self_cond, clip_denoised = clip_denoised) #得到了前向均值方差，
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0 #
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, shape, cond,cond_mask):
        ...
        for t in reversed(range(0, self.num_timesteps)):
            ...
            img, x_start = self.p_sample(img, t, cond,cond_mask)
        ...
        # times = get_schedule_jump(**{'jump_length': 5, 'jump_n_sample': 5, 'n_sample': 1, 't_T': 1000})
        # time_pairs = list(zip(times[:-1], times[1:]))
        # full_num=0
        # for t_last, t_cur in tqdm(time_pairs[:-1]):
        #     self_cond = x_start if self.self_condition else None
        #     if t_cur < t_last:
        #         img, x_start = self.p_sample(img, t_cur, cond, cond_mask,full_num, self_cond)
        #     else:
        #         batched_times = torch.ones((img.shape[0],), device=img.device, dtype=torch.long)*t_cur
        #         beta = extract(self.betas,batched_times,img.shape)
        ...
        img = unnormalize_to_zero_to_one(torch.clamp(img,-1,1))
        return img
    ...
    def p_losses(self, x_start, t, cond, noise = None):
        ...
        x = self.q_sample(x_start = x_start, t = t, noise = noise) #x=xt
    ...
    def forward(self, img, cond_img, *args, **kwargs):
        ...
        normed_cond_img=normalize_to_neg_one_to_one(cond_img)
        img = torch.cat((img,normed_cond_img),dim=1)
